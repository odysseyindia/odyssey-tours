---
title: Glenburn Tea Estate
translationKey: glenburn-tea-estate
bookthrough: ''
mailto: ''
address: Near Singritan
city: Darjeeling
postalcode: 734 101
phone: '-,'
mobile: ''
email: info@glenburnteaestate.com
website: http://www.glenburnteaestate.com/
note: ''
type: hotel
tags:
  - Hotels
category:
  - H
  - SH
  - WEB
showHotel: 0
starCategory: 0
checkout: ''
accessRail: ''
accessAir: ''
accessBus: ''
advantage: ''
rooms: 0
rates:
  - roomType: Burra Bungalow
    mealPlan: Fullboard
    mealPlanPT: ''
    currency: INR
    currencyPT: INR
    costSingle: 27555
    costSinglePT: 0
    costSingleAc: 0
    costSingleAcPT: 0
    costDouble: 46376
    costDoublePT: 0
    costDoubleAc: 0
    costDoubleAcPT: 0
    nett: 1
    nettPT: 0
    freeTransfer: 1
    extraBed: 11682
    extraBedPT: 0
    wef: '2020-10-01T00:00:00'
    wet: '2021-09-30T00:00:00'
    fromPax: 1
    toPax: 15
    git: 0
    serviceCharges: 0
    commission: 0
    commissionPT: 0
    commissionOnPlan: 0
    commissionOnPlanPT: 0
    serviceChargesOnPlan: 0
    blackout: 0
    dayOfTheWeek: 127
    defaultRoomType: Burra Bungalow
    defaultAc: 1
    specialPeriod: 0
    notes: >-
      Rs 3,894/- for child between 3 to 15 years.  There are 4 Burra Bungalow
      suites and 4 Water Lily Suites. The latter are only accessible by steps.
    agentCommission: 0
  - roomType: Water Lily Bungalow
    mealPlan: Fullboard
    mealPlanPT: ''
    currency: INR
    currencyPT: INR
    costSingle: 27555
    costSinglePT: 0
    costSingleAc: 0
    costSingleAcPT: 0
    costDouble: 46376
    costDoublePT: 0
    costDoubleAc: 0
    costDoubleAcPT: 0
    nett: 1
    nettPT: 0
    freeTransfer: 1
    extraBed: 11682
    extraBedPT: 0
    wef: '2020-10-01T00:00:00'
    wet: '2021-09-30T00:00:00'
    fromPax: 1
    toPax: 15
    git: 0
    serviceCharges: 0
    commission: 0
    commissionPT: 0
    commissionOnPlan: 0
    commissionOnPlanPT: 0
    serviceChargesOnPlan: 0
    blackout: 0
    dayOfTheWeek: 127
    defaultRoomType: Burra Bungalow
    defaultAc: 1
    specialPeriod: 0
    notes: >-
      Rs 3,894/- for child between 3 to 15 years.  There are 4 Burra Bungalow
      suites and 4 Water Lily Suites. The latter are only accessible by steps.
    agentCommission: 0
bookThrough: ''
gstType: 0
gstin: ''
www: http://www.glenburnteaestate.com/
web: 0
mailTo: ''
ranking: 1
---


















Glenburn is a beautiful tea plantation retreat above the banks of the river Rungeet, high in the Himalayas, overlooked by the mighty Kanchenjunga mountain range. Established in colonial times, it now offers a vineyard-like tea experience, where you can learn about tea in relaxed luxury, with outdoor sitouts, tea tastings, walking trails, massage treatments, fishing and more, amid lovingly restored planter's bungalows.